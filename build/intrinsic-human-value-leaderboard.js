...
async function loadTable() {
    ...
    for (const profile of profiles) {
        const followersPerFollow = (profile.followersCount ?? 0) / (profile.followsCount ?? 0);
        const followersPerPost = (profile.followersCount ?? 0) / (profile.postsCount ?? 0);
...
function loadChartData(loadedHandles) {
    ...
    const filteredHandles = sortedHandles.filter((handle) => checkedHandles.includes(handle));
    chartData = [
        [
            "Date",
    ...
    for (const date of allDates) {
        ...
        for (const handle of filteredHandles) {
            const r = items.find((i) => i.handle === handle && i.date === date)?.followers ||
                undefined;
            row.push(r);
        }
        chartData.push(row);
